...
def is_valid(x):
    if np.linalg.norm(np.array(x) - np.array([0.5, 0.5])) < 0.45:#simple circle bounds check
        return False
    return True


def sample_valid():#generate the random start and end for segments
    while True:
        x = np.random.rand(2)
        if is_valid(x):
            return x
...
if __name__ == "__main__":
    ...
    visualize: bool = args.visualize#should we visualize the data?

    planner = Planner(#make a new RRT planner
        [0, 0], [1, 1], is_valid, 1000, [0.04, 0.04], Algorithm.RRTConnect
    )
    #lower bound, upper bound, valid func, max size generated, validation box, algorithm
